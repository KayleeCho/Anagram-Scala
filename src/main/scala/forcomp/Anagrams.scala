...
object Anagrams extends AnagramsInterface {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    w.groupBy(c => c.toLower).map { case (c, s) => (c, s.length) }.toList.sortBy(_._1)
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] =
    ...
    case Nil => List(List())
    case (char, count) :: xs => {
      val combos = combinations(xs)
      (for {
        combo <- combos
        n <- 1 to count
      } yield (char, n) :: combo) ++ combos
    }
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences =  {
    ...
    x.foldLeft(List[(Char, Int)]())({
      case (acc, (char, count)) => {
        val remainder = count - ymap(char)
        if (remainder > 0) acc ++ List((char, remainder)) else acc
      }
    })
  }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def insideSentenceAnagram(occurrences: Occurrences): List[Sentence] = {
      if (occurrences.isEmpty) List(Nil)
      else for {
        combination <- combinations(occurrences)
        word <- dictionaryByOccurrences.getOrElse(combination, Nil)
        rest <- insideSentenceAnagram(subtract(occurrences, combination))
      } yield word :: rest
    }

    insideSentenceAnagram(sentenceOccurrences(sentence))
  }
}
